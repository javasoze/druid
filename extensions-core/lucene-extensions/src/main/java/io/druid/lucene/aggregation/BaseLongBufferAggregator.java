...
/**
 */
public abstract class BaseLongBufferAggregator implements BufferAggregator {
    private static final Pattern LONG_PAT = Pattern.compile("[-|+]?\\d+");

    private final DimensionSelector selector;

    public BaseLongBufferAggregator(DimensionSelector selector) {
        this.selector = selector;
    }

    protected long[] getValue(long defaultVal) {
        List row = selector.getIds();
